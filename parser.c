...
expr *parse_atom(token_buffer *tb) {
    ...
    case TOKEN_SYMBOL:
        ...
        //TODO add parsing supprt for chars
...
expr *parse_list(token_buffer *tb) {
    ...
    result -> value.pair.car = parse_expr(tb);
    if (tb -> buffer[tb -> index].tag == TOKEN_DOT) {
        ++ tb -> index;
        result -> value.pair.cdr = parse_expr(tb);
        if (tb -> buffer[tb -> index].tag != TOKEN_RPAR) return NULL;
        ++ tb -> index;
        return result;
    } else {
        expr *current = result;
